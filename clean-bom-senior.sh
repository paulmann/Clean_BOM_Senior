...
#   • Optional disabling of BOM or CRLF processing via CLI flags
#
# Use Cases:
...
#
# Examples:
#   ./clean-bom-senior.sh                                    # Full processing
#   ./clean-bom-senior.sh --no-bom-clear                     # Skip BOM removal
#   ./clean-bom-senior.sh --no-rn-normalize                  # Skip CRLF conversion
#   ./clean-bom-senior.sh --no-bom-clear --no-rn-normalize   # Skip both
#   ./clean-bom-senior.sh --dry-run                          # Preview mode
#
# Exit Codes:
...
EXAMPLES
    ...
    clean-bom-senior.sh --no-bom-clear              Skip BOM removal
    clean-bom-senior.sh --no-rn-normalize           Skip CRLF normalization
...
clean_file() {
	...
	# Build sed expression dynamically based on flags
	local sed_expr=""
	if [ "$NO_RN_NORMALIZE" -eq 0 ]; then
		sed_expr="s/\r$//"
	fi
	if [ "$NO_BOM_CLEAR" -eq 0 ]; then
		if [ -n "$sed_expr" ]; then
			sed_expr="$sed_expr; 1s/^\xef\xbb\xbf//"
		else
			sed_expr="1s/^\xef\xbb\xbf//"
		fi
	fi
	# Perform cleaning with dynamically built sed command
	if [ -n "$sed_expr" ]; then
		if ! sed -e "$sed_expr" "$backup" > "$temp"; then
		...
		# No operations to perform, just copy backup to temp
		cp "$backup" "$temp"
	fi
...
show_greeting() {
	if [ -t 2 ]; then
		...
		printf "CRLF normalization: %s\n" "$([ "$NO_RN_NORMALIZE" -eq 1 ] && echo "DISABLED" || echo "ENABLED")" >&2
	else
...
main() {
	...
	if [ "${#REMAINING_ARGS[@]}" -gt 0 ]; then
		...
		# Recursive processing of all supported files
